...
typedef ApiBuilder = DashboardApi Function(FirebaseUser user);

/// An app that displays a personalized dashboard.
class DashboardApp extends StatefulWidget {
  static DashboardApi _mockApiBuilder(FirebaseUser user) =>
      MockDashboardApi()..fillWithMockData();
  static DashboardApi _apiBuilder(FirebaseUser user) =>
...
class _SignInSwitcherState extends State<SignInSwitcher> {
  ...
  void _handleSignIn(FirebaseUser user) {
    widget.appState!.api = widget.apiBuilder!(user);
    Provider.of<FirebaseUser>(context, listen: false).updateUser(user);
    setState(() {
      _isSignedIn = true;
    });
  }
